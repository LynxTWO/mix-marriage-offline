"""Generate deterministic tiny WAV stems for fixtures/golden_path_small."""
...
def _kick_mono() -> list[float]:
    ...
    for i in range(frames):
        ...
        out.append(0.6 * env * fundamental + 0.06 * click)
    return out


def _snare_mono() -> list[float]:
    frames = _frame_count()
    ...
    for i in range(frames):
        ...
        body = math.sin(2.0 * math.pi * 190.0 * t)
        out.append((0.46 * noise + 0.18 * body) * env)
    return out
...
def _bass_mono() -> list[float]:
    ...
    for i in range(frames):
        ...
        harmonic = 0.3 * math.sin(2.0 * math.pi * 164.82 * t + 0.2)
...
def generate(out_dir: Path) -> None:
    ...
    _write_wav(out_dir / "snare.wav", channels=1, samples=_snare_mono())
...
def main() -> int:
    ...
    parser.add_argument(
        "--out-dir",
        default=None,
        help="Output directory for generated stems (default: fixture stems/).",
    )
    ...
    if args.out_dir:
        ...
        out_dir = Path(__file__).resolve().parent / "stems"
    generate(out_dir)
    return 0
