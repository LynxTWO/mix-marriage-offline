...
Done: rendered similarity fallback now uses a deterministic multi-step sequencer with explicit per-step reporting and receipt aggregation; default safe-render keeps partial artifacts/receipts plus explicit QA failure metadata when safe backoffs are exhausted instead of treating that state as an accidental hard stop.
...
- [x] The Tauri desktop app now stages a frozen `mmo` CLI as a sidecar,
