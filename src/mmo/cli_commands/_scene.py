...
def _run_render_run_command(
    *,
    ...
    targets = render_plan_payload.get("targets", [])
    print(
        json.dumps(
            {
                "plan_id": plan_id,
                "plan_out": plan_out_path.resolve().as_posix(),
                "report_out": report_out_path.resolve().as_posix(),
                "jobs": job_count,
