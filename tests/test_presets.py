...
class TestPresets(unittest.TestCase):
    ...
    def test_each_preset_validates_against_run_config_schema(self) -> None:
        ...
        index = load_preset_index(preset_root)
        for preset_entry in index.get("presets", []):
            preset_file = preset_root / str(preset_entry["file"])
    ...
    def test_list_presets_deterministic_sorted_order(self) -> None:
        ...
        first = list_presets(preset_root)
        second = list_presets(preset_root)
    ...
    def test_list_presets_filtering_deterministic_sorted_order(self) -> None:
        ...
        tag_first = list_presets(preset_root, tag="translation")
        tag_second = list_presets(preset_root, tag="translation")
        ...
        category_ids = [
            item.get("preset_id")
            for item in list_presets(preset_root, category="vibe")
        ]
        self.assertEqual(
            category_ids,
            [
