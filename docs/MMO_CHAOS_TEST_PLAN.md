...
The goal is to catch failures where a truth-first, artifact-first, cross-platform system is most likely to be embarrassed.
...
4. Render plan edge-case tests
5. Workspace / stale-artifact fixture tests
6. Stem-folder fixture tests

### Phase 3: Real-world Linux run
7. Build MMO for Linux
8. Run against a real test folder
9. Execute validate → analyze → scene → preflight → render → compare
10. Inspect artifacts and UX behavior for real-world drift
...
MMO should stay trustworthy when the user behaves like a tired, disorganized, overly optimistic human who swears the folder is "basically right."
