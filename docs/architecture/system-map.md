...
| Release outputs | `release.yml` builds a manual PDF, Python sdist and wheel, standalone CLI binaries, packaged Tauri bundles, and release artifacts for all supported platforms. The repo owns workflow order, artifact selection, smoke orchestration, install-state capture, and cleanup policy. GitHub runners, Windows cert-store import, `signtool`, Authenticode results, installer semantics, and hosted release behavior stay outside repo proof. |
| Docs site deployment | `pages.yml` uploads `site/` and deploys it to GitHub Pages on pushes to `main` that touch the site or the workflow itself. Current repo-local evidence treats `site/` as committed static content uploaded as-is, while GitHub owns runner and hosted serving behavior after upload. |
