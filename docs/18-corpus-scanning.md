...
Use `tools/stem_corpus_scan.py` to scan private stem folders and build a
names-only corpus for lexicon tuning.
...
- `--redact-paths`: store `basename` + hashed `rel_dir` instead of raw
  `rel_path`.
- `--max-files`: optional deterministic cap for fast sampling.
- `--include-folder-tokens`: include folder tokens in stats/suggestions
  (default: off; basename tokens only).
...
- `--min-confidence`: suggestions/per-role stats confidence floor (default:
  `0.80`).
...
- ignore built-in low-signal stopwords (`track`, `stem`, `mix`, `master`,
  `print`, `take`, `alt`, `ver`, `version`, etc).
...
Treat it as a draft only. Review each token before using it in production
lexicon rules. The merge-suggestions command never modifies the built-in common
lexicon — it only writes user lexicons.

## How to diff runs

After re-scanning with updated lexicons or thresholds, compare two stats JSON
...
- `token_frequency_top_delta` — tokens with changed counts, sorted by abs(delta)
  desc
