...
This document defines the minimum scene/render contract for MMO's mix-once,
render-many architecture.

Legacy note: Prefer `02-architecture.md`, `15-target-selection.md`, and
`18-channel-standards.md` for the current canonical docs.
...
- No automatic "creative intent generation" beyond explicit user intent plus
  bounded inference
...
- Safety limits with context-based thresholds (`suggest`, `auto_apply`,
  `render`)

Render contracts must rely on canonical ontology layout definitions to avoid
channel-order drift.

## B.1) Channel-ordering standard support

MMO now supports five channel-ordering standards at the I/O boundary: `SMPTE`,
`FILM`, `LOGIC_PRO`, `VST3`, and `AAF`. Every render contract records which
standard was requested.

### SMPTE / ITU-R (default)

The ordering baked into WAV, FLAC, WavPack, FFmpeg, and most DAW exports. This
is the MMO default for all file I/O.
...
- `LOGIC_PRO` ordering for Logic Pro / DTS-style layouts
- `VST3` ordering for Cubase / Nuendo 7.1+ layouts
- `AAF` ordering for metadata-driven interchange when an explicit layout variant
  is declared

### How to request a specific ordering standard
...
- WAV export uses a conservative `WAVEFORMATEXTENSIBLE` strategy: direct-out
  style channel mask (`mask=0`) to avoid false single-LFE semantics.
...
3. If gates fail and an allowed backoff exists, apply conservative backoff and
   re-render.
4. Stop after a deterministic maximum attempt count; do not loop endlessly.
5. Emit a deterministic attempt log describing what was tried and why the final
   state passed or blocked.

When similarity gates fail, renderer must back off or stop. When correlation
risk is high, renderer must reduce risk conservatively or stop. In both cases,
behavior must be deterministic and explainable.

## D) Position stereo properly

- Stereo is one render target among many.
- When scene plus intent plus layout contracts exist, they are the source of
  truth.
- Stereo assets can inform inference, but must not override explicit scene locks
  or user intent.
