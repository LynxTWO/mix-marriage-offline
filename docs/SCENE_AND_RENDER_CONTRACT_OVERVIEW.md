...
Legacy note:
Prefer `02-architecture.md`, `15-target-selection.md`, and
...
MMO now supports five channel-ordering standards at the I/O boundary:
...
- `AAF` ordering for metadata-driven interchange when an explicit layout
  variant is declared
...
- Render reports always carry canonical SPK channel order (`channel_order`)
  and channel count from the resolved layout contract.
...
- Reports include warnings + validation instructions when external
  players/DAWs may collapse or relabel `LFE2`.
...
- Emit artifacts with deterministic metadata
  (no wall-clock creation timestamps)
...
2. Evaluate similarity and safety gates, including downmix and correlation
   risk.
...
5. Emit a deterministic attempt log describing what was tried and why the
   final state passed or blocked.

When similarity gates fail, renderer must back off or stop.
When correlation risk is high, renderer must reduce risk conservatively or
stop.
In both cases, behavior must be deterministic and explainable.
...
- Stereo assets can inform inference, but must not override explicit scene
  locks or user intent.
