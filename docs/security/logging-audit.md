...
| --- | --- | --- | --- | --- | --- | --- |
| `gui/lib/mmo_cli_runner.mjs` | `159-181` | thrown `Error` text built from subprocess stderr | Full trimmed CLI stderr for every failed candidate, plus command labels and exit codes | Failing MMO CLI paths often include absolute project paths, session paths, render artifact paths, and other user-chosen local filesystem details. The helper forwards the raw stderr text instead of an allowlisted summary. | `conditional leak` | Yes. GUI bridge and local subprocess handoff are protected. |
| `gui/lib/rpc_process_client.mjs` | `89-119` | retained stderr tail appended to RPC shutdown errors | Up to five lines of recent RPC subprocess stderr in rejection messages | The RPC child can emit render, project, or environment errors that contain machine-local paths or user-selected project locations. The browser-side client replays that stderr tail verbatim. | `conditional leak` | Yes. GUI RPC surfaces are protected. |
...
| --- | --- | --- | --- | --- |
| GUI CLI failure forwarding in `gui/lib/mmo_cli_runner.mjs` | Yes | runtime | Replace full `stderr=` text with a short allowlist such as exit code, candidate label, `stderr_present`, and line count. Keep raw stderr in a local debug-only path if needed. | `cd gui && npm test` plus the nearest `gui/tests/mmo_cli_runner.test.mjs` cases that cover failed candidates. |
| RPC stderr tail forwarding in `gui/lib/rpc_process_client.mjs` | Yes | runtime | Replace appended stderr tails with a short summary or a bounded first-line allowlist that excludes paths and raw payload text. | `cd gui && npm test` plus `gui/tests/rpc_process_client.test.mjs`. |
...
| --- | --- | --- | --- | --- | --- | --- |
| `gui/lib/mmo_cli_runner.mjs`, `gui/lib/rpc_process_client.mjs` | The exact stderr payloads forwarded into GUI errors depend on runtime failures | The code clearly forwards raw stderr text, but the worst-case data class depends on which CLI or RPC path fails | Static review shows `stderr` text is copied into thrown or rejected error messages | GUI bridge | Run one failing render or project command in the dev shell with a path-rich project and inspect the surfaced error text | `needs runtime confirmation` |
