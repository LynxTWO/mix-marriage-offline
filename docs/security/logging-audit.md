...
| Project CLI JSON output in `_project.py` and scan CLI JSON output in `scan_session.py` | Yes | runtime | Phases 1 through 7 are implemented on this branch: `project show`, `project save`, `project load`, and `scan_session` all have shared-safe shell modes or defaults, while RPC `json`, project `json-local`, and explicit scan `json-local` keep the local contracts unchanged. Phases 6 and 7 only rename the shell-only full local project and scan profiles. They do not widen the payload or the GUI, RPC, or file-backed report contracts. Remaining work is a later decision about the explicit local project or scan paths and the file-backed scan paths, not the shell-facing defaults. | `tools/run_pytest.sh -q tests/test_cli_project_show.py tests/test_cli_project_load_save.py tests/test_cli_gui_rpc.py tests/test_scan_smoke.py tests/test_validation_wav_codec.py tests/test_scan_ffmpeg_basic.py tests/test_scan_ffprobe_layout.py tests/test_scan_truth_weighting_multiformat.py tests/test_truth_meters_optional_deps.py tests/test_cli_scan_lfe_audit.py tests/test_cli_project_build_gui.py`, plus `python3 tools/validate_contracts.py`. |
...
- `scan_session` and `mmo scan` now default shell stdout to the shared-safe
  profile. Use `--format json-local` only when local tooling needs the full
  local report, and use `--out` when a later stage needs the full file-backed
  artifact.
