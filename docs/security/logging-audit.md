...
| `src/mmo/cli_commands/_project.py`, `src/mmo/core/config.py` | `_project.py:233-311`, `613-686`; `config.py:235-258`, `303-309`, `380-386` | machine-readable CLI JSON stdout | Default `project show --format json` prints `project_dir` and per-artifact `absolute_path`. New `project show --format json-shared` drops those machine-local path fields. `project save` and `project load` still print `project_dir`, `session_path`, `scene_path`, and written path summaries. | These are intended product outputs, not hidden telemetry, but they are still sensitive when shell history, CI logs, or support captures stdout. The new shared-safe `project show` profile narrows one shell-facing path, but the default local contract and the save or load outputs still expose project and session filesystem structure directly. | `conditional leak` | Yes. Project and session control-plane paths can alter local project state. |
...
| Project CLI JSON output in `_project.py` | Yes | runtime | Phase 1 is implemented on this branch: `project show --format json-shared` strips `project_dir` and `absolute_path` while keeping the GUI and RPC `json` contract unchanged. Remaining work is a later packet for default `json`, `project save` and `project load`, and any scan-output follow-up. | `tools/run_pytest.sh -q tests/test_cli_project_show.py tests/test_cli_gui_rpc.py tests/test_cli_project_load_save.py` if present, plus `python3 tools/validate_contracts.py`. |
