...
| `src/mmo/cli_commands/_project.py`, `src/mmo/core/config.py` | `_project.py:233-375`, `665-791`; `config.py:238-314`, `317-389` | machine-readable CLI JSON stdout | Shell-facing `project show`, `project save`, and `project load` now default to shared-safe JSON. Explicit `--format json-local` and RPC `json` still print `project_dir`, `absolute_path`, `session_path`, `scene_path`, and written path summaries when local tooling needs them. | These are intended product outputs, not hidden telemetry, but they are still sensitive when shell history, CI logs, or support captures stdout. The shell-facing boundary is now narrower for the project commands, but the explicit local project path contract still exposes project and session filesystem structure directly. | `conditional leak` | Yes. Project and session control-plane paths can alter local project state. |
| `src/mmo/tools/scan_session.py`, `src/mmo/core/session.py`, `src/mmo/core/media_tags.py` | `scan_session.py:1309-1387`, `1548-1569`, `1617-1674`; `session.py:44-67`, `109-112`; `media_tags.py:261-267` | machine-readable CLI JSON stdout | Shell-facing scan stdout now only emits the shared-safe profile that drops `session.stems_dir`, per-stem `sha256`, `source_metadata`, `resolved_path`, `resolve_error_detail`, and file-hash evidence. File-backed `--out` still keeps the full local report including `session.stems_dir`, per-stem `file_path`, `sha256`, and media-tag-derived content. | This output still carries private creative-data context when local tooling writes the full report artifact, but the shell-facing stdout boundary is now narrower for issue threads and shared logs. The remaining risk sits in file-backed receipts and any shared handling of those outputs outside the repo. | `conditional leak` | No for the scan command itself, but the data class is high risk. |
...
| Project CLI JSON output in `_project.py` and scan CLI JSON output in `scan_session.py` | Yes | runtime | Phases 1 through 8 are implemented on this branch: `project show`, `project save`, `project load`, and `scan_session` all have shared-safe shell modes or defaults. RPC `json` and project `json-local` still keep the local project contracts unchanged. Phase 8 retires the full-local scan stdout profile entirely, so the full local scan contract now stays file-backed under `--out`. The remaining work is a later decision about whether project `json-local` or file-backed scan artifacts should narrow further, not the shell-facing defaults. | `tools/run_pytest.sh -q tests/test_cli_project_show.py tests/test_cli_project_load_save.py tests/test_cli_gui_rpc.py tests/test_scan_smoke.py tests/test_validation_wav_codec.py tests/test_scan_ffmpeg_basic.py tests/test_scan_ffprobe_layout.py tests/test_scan_truth_weighting_multiformat.py tests/test_truth_meters_optional_deps.py tests/test_cli_scan_lfe_audit.py tests/test_cli_project_build_gui.py`, plus `python3 tools/validate_contracts.py`. |
...
- `project show` now defaults to the shared-safe shell profile. Use
  `--format json-local` only when local tooling needs the full GUI or RPC
  path contract.
- `project save` and `project load` now default to the shared-safe profile for
  shell use. Use `--format json-local` only when local tooling truly needs the
  full machine-local path contract.
- `scan_session` and `mmo scan` now default shell stdout to the shared-safe
  profile. Use `--out` when local tooling needs the full local report artifact.
