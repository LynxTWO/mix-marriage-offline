...
| `src/mmo/cli_commands/_project.py`, `src/mmo/core/config.py` | `_project.py:233-375`, `665-791`; `config.py:238-314`, `317-389` | machine-readable CLI JSON stdout | Default `project show --format json` prints `project_dir` and per-artifact `absolute_path`. `project save` and `project load` now default to the shared-safe profile, while explicit `--format json` and RPC `json` still print `project_dir`, `session_path`, `scene_path`, and written path summaries. | These are intended product outputs, not hidden telemetry, but they are still sensitive when shell history, CI logs, or support captures stdout. The shell-facing boundary is now narrower for save and load, but the default local `project show` contract, the explicit local `json` path, and the wider scan output still expose project and session filesystem structure directly. | `conditional leak` | Yes. Project and session control-plane paths can alter local project state. |
...
| Project CLI JSON output in `_project.py` | Yes | runtime | Phases 1 through 3 are implemented on this branch: `project show --format json-shared`, `project save --format json-shared`, and `project load --format json-shared` exist, and `project save` or `project load` now default to the shared-safe profile while RPC keeps the local `json` contract unchanged. Remaining work is a later packet for shell-facing default `project show --format json` and any scan-output follow-up. | `tools/run_pytest.sh -q tests/test_cli_project_show.py tests/test_cli_project_load_save.py tests/test_cli_gui_rpc.py` if present, plus `python3 tools/validate_contracts.py`. |
