...
| Project CLI JSON output in `_project.py` | Yes | runtime | Phases 1 through 4 are implemented on this branch: `project show`, `project save`, and `project load` all have shared-safe JSON modes, and all three now default to the shared-safe shell contract while RPC keeps the local `json` contract unchanged. Remaining work is a later packet for scan-output follow-up or any future decision about the explicit local `json` path. | `tools/run_pytest.sh -q tests/test_cli_project_show.py tests/test_cli_project_load_save.py tests/test_cli_gui_rpc.py` if present, plus `python3 tools/validate_contracts.py`. |
