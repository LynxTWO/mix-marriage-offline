...
| `src/mmo/cli_commands/_project.py`, `src/mmo/core/config.py` | `_project.py:233-375`, `665-791`; `config.py:238-314`, `317-389` | machine-readable CLI JSON stdout | Default `project show --format json` prints `project_dir` and per-artifact `absolute_path`. `project save` and `project load` default `json` still print `project_dir`, `session_path`, `scene_path`, and written path summaries. New `json-shared` profiles now narrow `project show`, `project save`, and `project load` for shared logs. | These are intended product outputs, not hidden telemetry, but they are still sensitive when shell history, CI logs, or support captures stdout. The additive shared-safe profiles narrow the shell-facing path boundary, but the default local contract and the wider scan output still expose project and session filesystem structure directly. | `conditional leak` | Yes. Project and session control-plane paths can alter local project state. |
...
| Project CLI JSON output in `_project.py` | Yes | runtime | Phases 1 and 2 are implemented on this branch: `project show --format json-shared`, `project save --format json-shared`, and `project load --format json-shared` now strip `project_dir` and narrow shared path fields while keeping the default GUI and RPC `json` contract unchanged. Remaining work is a later packet for shell-facing default `json` and any scan-output follow-up. | `tools/run_pytest.sh -q tests/test_cli_project_show.py tests/test_cli_project_load_save.py tests/test_cli_gui_rpc.py` if present, plus `python3 tools/validate_contracts.py`. |
...
- Use `project show --format json-shared` when you need to share project
  metadata outside the local GUI or RPC path.
- Use `project save --format json-shared` or `project load --format
  json-shared` when shell logs or issue threads need a session summary without
  machine-local path roots.
