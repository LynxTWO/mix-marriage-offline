...
| `src/mmo/dsp/plugins/registry.py`, `src/mmo/plugins/subjective/*`, `src/mmo/core/binaural_target.py`, and `src/mmo/cli_commands/_renderers.py` | The subjective pack shapes shipped behavior outside the main manifest-loader path | A later plugin-authority pass could overclaim coverage if it treats manifest roots as the whole shipped plugin surface | `docs/review/bundled-plugin-review.md`; direct calls from `src/mmo/core/binaural_target.py` and `src/mmo/cli_commands/_renderers.py`; hard registration in `src/mmo/dsp/plugins/registry.py` | DSP or plugin maintainers | Decide whether to keep this as a DSP-side exception with explicit notes or pull it into a wider plugin-authority review | High |
