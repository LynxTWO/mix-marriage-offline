...
- `project show --format json-shared` now provides an additive shared-log-safe
  JSON profile that drops machine-local path fields, while the GUI and RPC path
  keeps the full local `json` contract.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Complete the remaining project and scan output boundary work | Project and scan output surfaces in `src/mmo/cli_commands/_project.py` and `src/mmo/tools/scan_session.py` | High | High | `project show`, `project save`, `project load`, and scan output expose absolute project paths, session paths, stem paths, hashes, and media-tag data. Those are product outputs locally, but they become telemetry once wrappers, CI, or support capture stdout. Phase 1 now adds `project show --format json-shared`, but the default local contract and the wider scan boundary still remain. | `docs/security/logging-audit.md` findings for `_project.py`, `src/mmo/tools/scan_session.py`, `src/mmo/core/session.py`, and `src/mmo/core/media_tags.py`; `docs/security/logging-audit.md` approval-status row `Project CLI JSON output in _project.py`; `docs/review/scenario-stress-test.md` section 5; `docs/architecture/coverage-ledger.md` row `Machine-readable product output and local trace escape`; phase-1 `project show` shared-output pass | verified | yes | project-output boundary review or approved redaction pass | Keep the next edit split by consumer path. Leave GUI and RPC on the local `json` contract, then decide whether default `json`, `project save` and `project load`, or scan output gets the next shared-safe boundary change. | `tools/run_pytest.sh -q tests/test_cli_project_show.py tests/test_cli_gui_rpc.py tests/test_cli_project_load_save.py tests/test_scan_smoke.py tests/test_cli_scan_lfe_audit.py`; rerun `python tools/validate_contracts.py`; review one shell `json-shared` sample and one GUI `json` sample to confirm the paths only move where intended. | CLI, tooling, or support maintainers | open |
...
| Confirm machine-readable output and trace escape channels | Machine-readable product output in `_project.py`, `src/mmo/tools/scan_session.py`, and `tools/agent/*` | High | High | The repo has verified path-bearing JSON and trace artifacts. It now also proves one local browser-visible `project.show` path, one manual issue-report channel, and no repo-owned workflow upload for these outputs. The remaining gap is how out-of-repo support or CI practice handles them. That still blocks safe prioritization between redaction, documentation, and operator guidance. | `docs/review/scenario-stress-test.md` section 5; `docs/unknowns/adversarial-pass.md` row for `_project.py`, `scan_session.py`, and `tools/agent/*`; `docs/unknowns/scenario-stress-test.md`; `docs/unknowns/logging-audit.md`; `docs/review/evidence-gap-check.md`; `.github/ISSUE_TEMPLATE/bug_report.yml`; GUI RPC inspection; workflow inspection of `.github/workflows/ci.yml` and `.github/workflows/release.yml` | inferred | no | output-boundary follow-up or approval-packet refresh | Separate the next pass into two parts: refresh the project and scan approval story with the verified local GUI path and issue-template channel, and keep `tools/agent/*` on the deferred artifact-contract track unless a shared workflow is proven. | Review any remaining maintainer or support guidance outside the repo; if a later code fix follows, reuse the verification plans from the project-output backlog item and `tests/test_agent_harness.py` for the deferred agent-contract track. | CLI, tooling, or support maintainers | open |
