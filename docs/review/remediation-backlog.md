...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Complete the remaining project and scan output boundary work | Project and scan output surfaces in `src/mmo/cli_commands/_project.py` and `src/mmo/tools/scan_session.py` | High | High | `project show`, `project save`, `project load`, and scan output expose absolute project paths, session paths, stem paths, hashes, and media-tag data. Those are product outputs locally, but they become telemetry once wrappers, CI, or support capture stdout. Shared-safe profiles now exist for all three project commands, and save or load now defaults to the shared-safe shell contract, but the default local `project show` contract and the wider scan boundary still remain. | `docs/security/logging-audit.md` findings for `_project.py`, `src/mmo/tools/scan_session.py`, `src/mmo/core/session.py`, and `src/mmo/core/media_tags.py`; `docs/security/logging-audit.md` approval-status row `Project CLI JSON output in _project.py`; `docs/review/scenario-stress-test.md` section 5; `docs/architecture/coverage-ledger.md` row `Machine-readable product output and local trace escape`; phase-1 through phase-3 project shared-output passes | verified | yes | project-output boundary review or approved redaction pass | Keep the next edit split by consumer path. Leave GUI and RPC on the local `json` contract, then decide whether shell-facing default `project show --format json` or scan output gets the next boundary change. | `tools/run_pytest.sh -q tests/test_cli_project_show.py tests/test_cli_project_load_save.py tests/test_cli_gui_rpc.py tests/test_scan_smoke.py tests/test_cli_scan_lfe_audit.py`; rerun `python tools/validate_contracts.py`; review one shell shared-safe sample and one GUI `json` sample to confirm the paths only move where intended. | CLI, tooling, or support maintainers | open |
