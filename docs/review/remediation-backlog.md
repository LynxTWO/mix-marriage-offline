...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Complete the remaining project and scan output boundary work | Project and scan output surfaces in `src/mmo/cli_commands/_project.py` and `src/mmo/tools/scan_session.py` | High | High | `project show`, `project save`, `project load`, and scan output expose absolute project paths, session paths, stem paths, hashes, and media-tag data. Those are product outputs locally, but they become telemetry once wrappers, CI, or support capture stdout. The shell-facing project commands now default to shared-safe output, so the remaining open work is the wider scan boundary and any future decision about the explicit local `json` contract. | `docs/security/logging-audit.md` findings for `_project.py`, `src/mmo/tools/scan_session.py`, `src/mmo/core/session.py`, and `src/mmo/core/media_tags.py`; `docs/security/logging-audit.md` approval-status row `Project CLI JSON output in _project.py`; `docs/review/scenario-stress-test.md` section 5; `docs/architecture/coverage-ledger.md` row `Machine-readable product output and local trace escape`; phase-1 through phase-4 project shared-output passes | verified | yes | scan-output boundary review or approved redaction pass | Keep GUI and RPC on the local `json` contract. Treat the next code change as a scan-only packet unless new evidence proves the explicit local project JSON contract should narrow too. | `tools/run_pytest.sh -q tests/test_cli_project_show.py tests/test_cli_project_load_save.py tests/test_cli_gui_rpc.py tests/test_scan_smoke.py tests/test_cli_scan_lfe_audit.py`; rerun `python tools/validate_contracts.py`; review one shell shared-safe sample and one GUI `json` sample to confirm the project paths only move where intended before touching scan output. | CLI, tooling, or support maintainers | open |
